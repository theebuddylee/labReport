...
PRIMARY_COLOR = "#06B6D4"      # e.g., a blue hue for optimal range (used in legend and elsewhere)
SECONDARY_COLOR = "#737373"    # e.g., a grayish hue for clinical range
...
if uploaded_file is not None:
    ...
    if text:
        ...
        for match in results:
            ...
            # Post-process: keep only text before the first digit.
        ...
        if extracted_data:
            ...
            # Convert 'Value' column to numeric where possible, keeping original values otherwise
            df_extracted["Value"] = pd.to_numeric(df_extracted["Value"], errors='coerce')

            # Convert NaNs to empty strings explicitly to prevent serialization errors
...
def create_range_chart(marker_name, units, clinical_range, optimal_range, user_value):
    ...
    Creates a horizontal stacked bar chart showing:
      - Clinical Range: a solid bar in SECONDARY_COLOR (top).
      - Optimal Range: a solid PRIMARY_COLOR bar (bottom).
      - A vertical line in vibrant_red for the user value.
    """
    ...
    # Adjust bar height to half its original size
    bar_height = 0.075
    optimal_y = 0.0
    clinical_y = optimal_y + bar_height

    # Clinical range in SECONDARY_COLOR
    ...
    # Optimal range in PRIMARY_COLOR (no gradient)
    ...
    # User value as a vertical red line
...
template_pdf_path = "LabResults.pdf"
...
def merge_pdfs(template_path, generated_path, output_path):
    with fitz.open(template_path) as template_pdf:
        ...
        # Append the last page from the template
        total_pages = len(template_pdf)
        for page_num in range(total_pages - 1, total_pages):
            output_pdf.insert_pdf(template_pdf, from_page=page_num, to_page=page_num)
